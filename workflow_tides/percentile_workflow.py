...
from tidal_median_workflow import tidal_workflow
...
class PercentileTask(luigi.Task):
    ...
    def load_offsets(self):
        fname="/g/data2/v10/ARG25-tidal-analysis/test/bb_high/final_datafile_"+ self.lat_lon
        print "loading offset file ", fname
        try:
            ...
            print "Could not read a offsetfile file ", fname
    ...
    def run(self):
        ...
        low_off, high_off = self.load_offsets()
        #low_off = int(CONFIG.get('internals', 'low_offset'))                            
        #high_off = int(CONFIG.get('internals', 'high_offset'))                            
        print "low off and high offset ", low_off, high_off
        # Execute
        tidal_workflow(db_tiles, percentile=self.pct, xtile=xtile, 
                    ytile=ytile, low_off=low_off, high_off=high_off, out_fnames=out_fnames)
